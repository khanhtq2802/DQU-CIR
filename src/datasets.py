...
class FashionIQ(torch.utils.data.Dataset):
    def __init__(self, path, category, transform=None, split='val-split'):
        ...
        self.split = split
        # begin khanh
        image_splits_category_path = os.path.join(self.image_dir, f"{self.category}")
        list_file = os.listdir(image_splits_category_path)
        self.list_image = [file.replace(' .jpg', '') for file in list_file]
        # end khanh
    ...
    def train_init_process(self):
        ...
        with open(os.path.join(self.caption_dir, "cap.{}.{}.json".format(self.category, 'train')), 'r') as f:
            raw_ref_captions = json.load(f)
        ref_captions = []
        for i in range(len(raw_ref_captions)):
            target = raw_ref_captions[i]['target']
            candidate = raw_ref_captions[i]['candidate']
            if (target in self.list_image) and (candidate in self.list_image):
                ref_captions.append(raw_ref_captions[i])
        # end khanh
